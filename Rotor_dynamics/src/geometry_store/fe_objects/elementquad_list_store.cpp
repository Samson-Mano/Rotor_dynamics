...
void elementquad_list_store::add_elementquadrilateral(int& quad_id, node_store* nd1, node_store* nd2, node_store* nd3, node_store* nd4)
{
	...
	glm::vec2 node_pt1 = (*nd1).node_pt; // Node pt 1
	glm::vec2 node_pt2 = (*nd2).node_pt; // Node pt 2
	glm::vec2 node_pt3 = (*nd3).node_pt; // Node pt 3
	glm::vec2 node_pt4 = (*nd4).node_pt; // Node pt 4
	...
	glm::vec2 midpt = glm::vec2((node_pt1.x + node_pt2.x + node_pt3.x + node_pt4.x) / 4.0f,
	...
	glm::vec2 shrunk_node_pt4 = geom_parameters::linear_interpolation(midpt, node_pt4, shrink_factor);
...
void elementquad_list_store::add_selection_quadrilaterals(const std::vector<int>& selected_element_ids)
{
	...
	for (const auto& it : selected_element_ids)
	{
		...
		glm::vec2 node_pt1 = elementquadMap[it].nd1->node_pt; // Node pt 1
		glm::vec2 node_pt2 = elementquadMap[it].nd2->node_pt; // Node pt 2
		glm::vec2 node_pt3 = elementquadMap[it].nd3->node_pt; // Node pt 3
		glm::vec2 node_pt4 = elementquadMap[it].nd3->node_pt; // Node pt 4


		glm::vec2 midpt = glm::vec2((node_pt1.x + node_pt2.x + node_pt3.x + node_pt4.x) / 4.0f, // mid pt x
...
void elementquad_list_store::update_material(const std::vector<int> selected_element_quad, const int& material_id)
{
	// Update the material ID
	for (const int& it : selected_element_quad)
	{
...
void elementquad_list_store::paint_tri_material_id()
{
...
void elementquad_list_store::update_geometry_matrices(bool set_modelmatrix, bool set_pantranslation, bool set_zoomtranslation, bool set_transparency, bool set_deflscale)
{
...
void elementquad_list_store::update_material_id_labels()
{
	...
	for (auto it = elementquadMap.begin(); it != elementquadMap.end(); ++it)
	{
		...
		glm::vec2 nd_pt1 = elementquad.nd1->node_pt;
		glm::vec2 nd_pt2 = elementquad.nd2->node_pt;
		glm::vec2 nd_pt3 = elementquad.nd3->node_pt;
		glm::vec2 nd_pt4 = elementquad.nd4->node_pt;

		// Calculate the midpoint of the quadrilateral
		glm::vec2 quad_mid_pt = glm::vec2((nd_pt1.x + nd_pt2.x + nd_pt3.x + nd_pt4.x) / 4.0f,
										 (nd_pt1.y + nd_pt2.y + nd_pt3.y + nd_pt4.y) / 4.0f);

		// Add the material ID
		temp_color = geom_parameters::get_standard_color(elementquad.material_id);
		temp_str = " M = " + std::to_string(elementquad.material_id);
		element_materialid.add_text(temp_str, quad_mid_pt, glm::vec2(0), temp_color, 0, true, false);
	}
